...
public class SentenceBuilderApp extends Application {
    ...
    // Parser is pure; ImportService owns the atomic database transaction.
    ...
    private void logStartupDatabaseStatus() {
        try (Connection conn = WordDb.openConnection()) {
            ...
            log(
                "Database: connected. Import a .txt file first, then build a draft with generation and autocomplete."
            );
    ...
    private void refreshDraftSuggestionPreview() {
        ...
        try {
            AutocompleteViewState state = autocompleteController.onWordCommitted(
                lastWord,
                ' ',
                suggestionLimitSpinner == null || suggestionLimitSpinner.getValue() == null
                    ? 5
                    : suggestionLimitSpinner.getValue().intValue()
            );
            renderAutocompleteState(state);
        } catch (SQLException exception) {
    ...
    private boolean databaseHasWorkspaceData() {
        try (Connection conn = WordDb.openConnection()) {
            try (java.sql.PreparedStatement ps = conn.prepareStatement(
                "SELECT 1 FROM next_word LIMIT 1"
            );
    ...
    private void loadStartupSummaryFromDatabase() {
        try (Connection conn = WordDb.openConnection();
            ...
            ParseResult pseudo = new ParseResult();
            pseudo.setFileName(rs.getString("file_name"));
            pseudo.setTotalWords(rs.getInt("word_count"));
            pseudo.setTotalSentences(rs.getInt("sentence_count"));
            pseudo.setTotalParagraphs(0);
            updateSummary(pseudo);
        } catch (SQLException e) {
            log("Startup summary load failed: " + e.getMessage());
        }
    }
}
